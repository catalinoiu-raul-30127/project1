...
// console.log(dates.year)
  const uniqueUsers = [...new Set(data.map(item => item.from))]; 
  const uniqueMonths=[...new Set(data.map(item => new Date(item.date).getMonth()))];
  const uniqueDays=[...new Set(data.map(item => new Date(item.date).getUTCDate()))];
  

  console.log(uniqueDays)
  let length=uniqueMonths.length*uniqueDays.length;
  console.log(length)
  

  for(let i=0;i<uniqueUsers.length;i++)
    {
        process.stdout.write(uniqueUsers[i]+":"+"  [");
...
//     let day=new Date(value.date).getDay();
//     // console.log(day);
    
       
    
//    // const uniqueMonths=[...new Set(month)];
//     //process.stdout.write(uniqueMonths+" "+year+"\n");
//    // for(i=0;i<=30;i++)
//    // {
